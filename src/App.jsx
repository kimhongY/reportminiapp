...
// ─── STORAGE ──────────────────────────────────────────────────────────────────
const LS = {
  getReports: ()=>{ try{return JSON.parse(localStorage.getItem("bra_rpts")||"[]")}catch{return[]} },
  saveReport: (r)=>{ const a=LS.getReports(); a.push({...r,id:Date.now(),ts:new Date().toISOString()}); localStorage.setItem("bra_rpts",JSON.stringify(a)) },
  getUsers:   ()=>{ try{const s=localStorage.getItem("bra_usrs"); return s?JSON.parse(s):[]}catch{return []} },
  saveUsers:  (u)=>localStorage.setItem("bra_usrs",JSON.stringify(u)),
  getBg:      ()=>{ try{return JSON.parse(localStorage.getItem("bra_bg")||"null")||BG_PRESETS[0]}catch{return BG_PRESETS[0]} },
  saveBg:     (b)=>localStorage.setItem("bra_bg",JSON.stringify(b)),
  getActivities: ()=>{ try{return JSON.parse(localStorage.getItem("bra_acts")||"[]")}catch{return[]} },
  saveActivities:(a)=>localStorage.setItem("bra_acts",JSON.stringify(a)),
};
...
:root{
  --acc:${accent};
  --acc2:color-mix(in srgb,${accent} 70%,white);
  --bg0:${a}; --bg1:${b}; --bg2:${c};
  --glass:rgba(255,255,255,.055);
  --glass2:rgba(255,255,255,.09);
  --border:rgba(255,255,255,.09);
  --border2:rgba(255,255,255,.15);
  --t0:#f0f6ff; --t1:rgba(200,220,255,.7);
  --t2:rgba(160,195,255,.42); --t3:rgba(130,175,255,.22);
}
...
.app{
  max-width:430px;margin:0 auto;height:100vh;min-height:0;
  display:flex;flex-direction:column;position:relative;overflow:hidden;
  background:
    radial-gradient(ellipse 160% 90% at 20% -10%,color-mix(in srgb,var(--acc) 12%,transparent) 0%,transparent 55%),
...
.brand-text h1{font-family:'Moul',serif;font-size:21px;color:#fff;letter-spacing:.04em}
...
.ii{position:absolute;left:13px;top:50%;transform:translateY(-50%);font-size:15px;pointer-events:none;opacity:.35}
...
.btn-green{background:linear-gradient(135deg,#059669,#047857);color:#fff;box-shadow:0 4px 18px rgba(5,150,105,.3);margin-top:2px}
...
.btn-orange{background:linear-gradient(135deg,#f97316,#c2410c);color:#fff;box-shadow:0 4px 18px rgba(249,115,22,.28)}
.btn-orange:hover{filter:brightness(1.1);transform:translateY(-1px)}
.btn-purple{background:linear-gradient(135deg,#8b5cf6,#6d28d9);color:#fff;box-shadow:0 4px 18px rgba(139,92,246,.28)}
...
.hav{width:40px;height:40px;border-radius:13px;flex-shrink:0;border:1.5px solid var(--border2);display:flex;align-items:center;justify-content:center;font-size:19px;box-shadow:inset 0 1px 0 rgba(255,255,255,.12)}
...
.sc{background:var(--glass);border:1px solid var(--border);border-radius:18px;padding:16px 17px;backdrop-filter:blur(12px);box-shadow:inset 0 1px 0 rgba(255,255,255,.08);display:flex;flex-direction:column;gap:4px}
.sn{font-size:28px;font-weight:800;line-height:1;letter-spacing:-.03em}
.sl{font-size:9px;font-weight:700;color:var(--t2);text-transform:uppercase;letter-spacing:.08em;margin-top:3px}
...
.ri:hover{background:rgba(255,255,255,.065)}
...
.rtm{font-size:10px;color:var(--t3)}
.rus{font-size:11px;color:var(--t2)}
.rbd{font-size:11px;color:var(--t2);line-height:1.8;white-space:pre-wrap;margin-top:3px}
...
.ui:hover{background:rgba(255,255,255,.028)}
...
.un{font-size:13px;font-weight:600;color:var(--t0)}
.uro{font-size:10px;color:var(--t2);margin-top:1px}
...
.cprow{display:grid;grid-template-columns:1fr 1fr;gap:10px}
.cpw{display:flex;flex-direction:column;gap:5px}
...
.act-type.booth{background:rgba(251,146,60,.15);color:#fb923c}
.act-type.roadshow{background:rgba(139,92,246,.15);color:#a78bfa}
.act-title{font-size:14px;font-weight:700;color:var(--t0)}
.act-meta{font-size:11px;color:var(--t2);display:flex;flex-direction:column;gap:3px}
.act-meta span{display:flex;align-items:center;gap:5px}
.act-members{display:flex;flex-wrap:wrap;gap:6px;margin-top:4px}
.act-member{font-size:10px;font-weight:600;padding:3px 9px;border-radius:20px;background:rgba(255,255,255,.07);border:1px solid var(--border);color:var(--t1)}
.act-member.pending{background:rgba(245,158,11,.1);border-color:rgba(245,158,11,.25);color:#fbbf24}
.act-member.approved{background:rgba(16,185,129,.1);border-color:rgba(16,185,129,.25);color:#34d399}
...
.req-name{font-size:12px;font-weight:600;color:var(--t0)}
.req-sub{font-size:10px;color:var(--t2)}
.btn-approve{background:rgba(16,185,129,.15);border:1px solid rgba(16,185,129,.3);color:#34d399;padding:5px 12px;font-size:11px;border-radius:8px;cursor:pointer;font-weight:700;font-family:inherit;transition:all .15s;width:auto}
.btn-approve:hover{background:rgba(16,185,129,.28)}
.btn-reject{background:rgba(239,68,68,.1);border:1px solid rgba(239,68,68,.22);color:#fca5a5;padding:5px 12px;font-size:11px;border-radius:8px;cursor:pointer;font-weight:700;font-family:inherit;transition:all .15s;width:auto}
...
function GC({icon,title,sub,children,accent}){
  return(
    <div className="gc">
      <div className="gch">
        <div className="gci" style={accent?{background:`color-mix(in srgb,${accent} 14%,transparent)`,borderColor:`color-mix(in srgb,${accent} 25%,transparent)`}:{}}>
          {icon}
        </div>
...
// ─── REPORT LIST (reusable) ───────────────────────────────────────────────────
function ReportList({reports,showFull=true}){
  ...
  return(
    <>
      <div className="tabs" style={{flexWrap:"wrap"}}>
        ...
        {types.map(t=>(
          <button key={t} className={`tab ${filter===t?"on":""}`} onClick={()=>sf(t)}>{t}</button>
        ))}
      </div>
      <div className="gc">
        <div className="gcb">
          ...
          {filtered.slice().reverse().map(r=>(
            <div key={r.id} className="ri">
              ...
              {showFull&&<div className="rbd">{r.msg}</div>}
            </div>
          ))}
        </div>
      </div>
...
function Login({onLogin}){
  ...
  const go=async()=>{
    ...
    if(matched){
      ...
      if(ok){sl(false);se("");onLogin(matched);return;}
    }
    ...
    <div className="lw">
      <div className="logo-shell">
        ...
        <div className="brand-text">
          <h1>Branch Report</h1>
          <div className="sub">Telegram Mini App</div>
...
function Header({user,onLogout}){
  ...
  return(
    <div className="hdr">
      <div className="hav" style={{background:`color-mix(in srgb,${cfg.color||"#fff"} 14%,transparent)`,borderColor:`color-mix(in srgb,${cfg.color||"#fff"} 24%,transparent)`}}>
        {cfg.icon||"👤"}
      </div>
...
function Teller({user}){
  ...
  const submit=async()=>{
    ...
    LS.saveReport({type:"Teller",user:user.display,data:f,msg});
...
function CSASup({user}){
  ...
  const [f,sf]=useState(e); const [toast,stt]=useState("");
  const s=(k,v)=>sf(p=>({...p,[k]:v}));
  const myReports=LS.getReports().filter(r=>r.type==="CSA"||r.type==="CSA_Officer");
  const submit=async()=>{
    ...
    LS.saveReport({type:"CSA",user:user.display,data:f,msg});
    await sendTelegram(msg); sf(e); stt("Report បានផ្ញើ!");
  };
  return(<>{toast&&<Toast msg={toast} onDone={()=>stt("")}/>}
    <div className="sb">
      <div className="tabs">
        {[["report","📝 Report"],["view","📋 ទិន្នន័យ"]].map(([k,l])=>(
          <button key={k} className={`tab ${tab===k?"on":""}`} onClick={()=>st(k)}>{l}</button>))}
      </div>
      {tab==="report"&&(
        <GC icon="🧾" title="Supervisor CSA Report" sub="ប្រចាំថ្ងៃ" accent="#34d399">
          <F label="Staff Name" ph="ឈ្មោះ Staff..." val={f.staffName} set={v=>s("staffName",v)} icon="👤"/>
          <div className="r2">
            <F label="New CIF" ph="ចំនួន..." val={f.newCif} set={v=>s("newCif",v)} nb/>
            <F label="New KHQR" ph="ចំនួន..." val={f.newKhqr} set={v=>s("newKhqr",v)} nb/>
          </div>
          <div className="r2">
            <F label="Deposit" ph="ចំនួន..." val={f.deposit} set={v=>s("deposit",v)} nb/>
            <F label="IA" ph="ចំនួន..." val={f.ia} set={v=>s("ia",v)} nb/>
          </div>
          <button className="btn btn-green" onClick={submit}>📤 Submit Report</button>
        </GC>
      )}
      {tab==="view"&&(
        <>
          <div className="sg">
            <div className="sc"><div className="sn" style={{color:"#34d399"}}>{myReports.filter(r=>r.type==="CSA").length}</div><div className="sl">CSA Sup Reports</div></div>
            <div className="sc"><div className="sn" style={{color:"#6ee7b7"}}>{myReports.filter(r=>r.type==="CSA_Officer").length}</div><div className="sl">CSA Officer Reports</div></div>
          </div>
          <ReportList reports={myReports} showFull={true}/>
        </>
      )}
    </div></>);
}
...
function LoanSup({user}){
  ...
  const myReports=LS.getReports().filter(r=>r.type==="Loan"||r.type==="Loan_Officer");
  const submit=async()=>{
    ...
    LS.saveReport({type:"Loan",user:user.display,data:f,msg});
    await sendTelegram(msg); sf(e); stt("Report បានផ្ញើ!");
  };
  return(<>{toast&&<Toast msg={toast} onDone={()=>stt("")}/>}
    <div className="sb">
      ...
      {tab==="report"&&(<>
        <GC icon="💳" title="Supervisor Loan Report" sub="ប្រចាំសប្ដាហ៍" accent="#a78bfa">
          ...
          <div className="r2">
            <F label="ពីថ្ងៃទី" ph="" val={f.df} set={v=>s("df",v)} nb type="date"/>
            <F label="ដល់ថ្ងៃទី" ph="" val={f.dt} set={v=>s("dt",v)} nb type="date"/>
          </div>
        </GC>
        <GC icon="📅" title="ចំនួន Loan ប្រចាំថ្ងៃ" sub="KHR / USD" accent="#a78bfa">
          {days.map((d,i)=>(
            <div key={d.khr}>
              ...
              <div style={{fontSize:11,fontWeight:700,color:"var(--acc2)",marginBottom:7}}>{d.lbl}</div>
              <div className="r2">
                <F label="KHR" ph="0" val={f[d.khr]} set={v=>s(d.khr,v)} nb/>
                <F label="USD" ph="0" val={f[d.usd]} set={v=>s(d.usd,v)} nb/>
              </div>
            </div>
          ))}
        </GC>
      ...
      {tab==="view"&&(
        <>
          <div className="sg">
            <div className="sc"><div className="sn" style={{color:"#a78bfa"}}>{myReports.filter(r=>r.type==="Loan").length}</div><div className="sl">Loan Sup Reports</div></div>
            <div className="sc"><div className="sn" style={{color:"#c084fc"}}>{myReports.filter(r=>r.type==="Loan_Officer").length}</div><div className="sl">Loan Officer Reports</div></div>
          </div>
          <ReportList reports={myReports} showFull={true}/>
        </>
      )}
...
function LoanOfficer({user}){
  ...
  const submit=async()=>{
    const rows=days.map(d=>`• ${d.lbl}: KHR <b>${f[d.khr]||"—"}</b> | USD <b>${f[d.usd]||"—"}</b>`).join("\n");
    const msg=`📄 <b>Loan Officer Report</b>\n👤 Loan Name: <b>${user.display}</b>\n📅 ${f.df} → ${f.dt}\n\n${rows}\n\n👤 ${user.display}`;
    LS.saveReport({type:"Loan_Officer",user:user.display,data:f,msg});
    await sendTelegram(msg); sf(e); st(true);
  };
  return(<>{t&&<Toast msg="Report បានផ្ញើ!" onDone={()=>st(false)}/>}
    <div className="sb">
      <GC icon="📄" title="Loan Officer Report" sub={user.display} accent="#c084fc">
        <div style={{background:"rgba(192,132,252,.08)",border:"1px solid rgba(192,132,252,.2)",borderRadius:10,padding:"9px 13px",fontSize:12,color:"#c084fc",fontWeight:600}}>
          👤 Loan Name: {user.display}
        </div>
        <div className="r2">
          <F label="ពីថ្ងៃទី" ph="" val={f.df} set={v=>s("df",v)} nb type="date"/>
          <F label="ដល់ថ្ងៃទី" ph="" val={f.dt} set={v=>s("dt",v)} nb type="date"/>
      ...
      <ActivityViewer user={user} canEdit={false} canJoin={true}/>
    </div></>);
}
...
function CSAOfficer({user}){
  ...
  const submit=async()=>{
    ...
    LS.saveReport({type:"CSA_Officer",user:user.display,data:f,msg});
    await sendTelegram(msg); sf(e); st(true);
  };
  return(<>{t&&<Toast msg="Report បានផ្ញើ!" onDone={()=>st(false)}/>}
    <div className="sb">
      <GC icon="🪪" title="CSA Officer Report" sub={user.display} accent="#6ee7b7">
        <div style={{background:"rgba(110,231,183,.08)",border:"1px solid rgba(110,231,183,.2)",borderRadius:10,padding:"9px 13px",fontSize:12,color:"#6ee7b7",fontWeight:600}}>
          👤 Staff Name: {user.display}
        </div>
        <div className="r2">
          <F label="New CIF" ph="ចំនួន..." val={f.newCif} set={v=>s("newCif",v)} nb/>
          <F label="New KHQR" ph="ចំនួន..." val={f.newKhqr} set={v=>s("newKhqr",v)} nb/>
...
function MAKHQR({user}){
  ...
  const s=(k,v)=>sf(p=>({...p,[k]:v}));
  const submit=async()=>{
    const msg=`📱 <b>MA KHQR Report</b>\n👤 MA: <b>${f.name||"—"}</b>\n📅 ${f.date}\n\n<b>▸ New KHQR:</b> KHR ${f.nK||"—"} | USD ${f.nU||"—"}\n<b>▸ Reprint KHQR:</b> KHR ${f.rK||"—"} | USD ${f.rU||"—"}\n<b>▸ Sound Box:</b> KHR ${f.sK||"—"} | USD ${f.sU||"—"}\n\n👤 ${user.display}`;
    LS.saveReport({type:"MA_KHQR",user:user.display,data:f,msg});
    await sendTelegram(msg); sf(e); st(true);
  };
  return(<>{t&&<Toast msg="Report បានផ្ញើ!" onDone={()=>st(false)}/>}
    <div className="sb">
      <GC icon="📱" title="MA KHQR Report" sub="KHQR Onboard" accent="#22d3ee">
        <div className="r2">
          <F label="MA Name" ph="ឈ្មោះ MA..." val={f.name} set={v=>s("name",v)} nb/>
          <F label="ថ្ងៃទី" ph="" val={f.date} set={v=>s("date",v)} nb type="date"/>
        </div>
        <SBox label="New KHQR Onboard"><div className="r2"><F label="KHR" ph="0" val={f.nK} set={v=>s("nK",v)} nb/><F label="USD" ph="0" val={f.nU} set={v=>s("nU",v)} nb/></div></SBox>
        <SBox label="Reprint KHQR Onboard"><div className="r2"><F label="KHR" ph="0" val={f.rK} set={v=>s("rK",v)} nb/><F label="USD" ph="0" val={f.rU} set={v=>s("rU",v)} nb/></div></SBox>
        <SBox label="Sound Box Onboard"><div className="r2"><F label="KHR" ph="0" val={f.sK} set={v=>s("sK",v)} nb/><F label="USD" ph="0" val={f.sU} set={v=>s("sU",v)} nb/></div></SBox>
        <button className="btn btn-green" onClick={submit}>📤 Submit Report</button>
      </GC>
    </div></>);
}
...
function MSKHQR({user}){
  ...
  const submit=async()=>{
    ...
    LS.saveReport({type:"MS_KHQR",user:user.display,data:f,msg});
    await sendTelegram(msg); sf(e); st(true);
  };
  return(<>{t&&<Toast msg="Report បានផ្ញើ!" onDone={()=>st(false)}/>}
    <div className="sb">
      <GC icon="🔖" title="MS KHQR Report" sub="KHQR Onboard" accent="#f472b6">
        <div className="r2">
          <F label="MS Name" ph="ឈ្មោះ MS..." val={f.name} set={v=>s("name",v)} nb/>
          <F label="ថ្ងៃទី" ph="" val={f.date} set={v=>s("date",v)} nb type="date"/>
        </div>
        <SBox label="New KHQR Onboard"><div className="r2"><F label="KHR" ph="0" val={f.nK} set={v=>s("nK",v)} nb/><F label="USD" ph="0" val={f.nU} set={v=>s("nU",v)} nb/></div></SBox>
        <SBox label="Reprint KHQR Onboard"><div className="r2"><F label="KHR" ph="0" val={f.rK} set={v=>s("rK",v)} nb/><F label="USD" ph="0" val={f.rU} set={v=>s("rU",v)} nb/></div></SBox>
...
// ─── ACTIVITY VIEWER (shared) ─────────────────────────────────────────────────
function ActivityViewer({user,canEdit=false,canJoin=false,canApprove=false}){
  const [acts,sa]=useState(LS.getActivities);
  const [toast,stt]=useState("");
  const reload=()=>sa(LS.getActivities());

  const requestJoin=(actId)=>{
    const date=prompt("ថ្ងៃដែលចូលរួម (YYYY-MM-DD):");
    if(!date)return;
    const updated=acts.map(a=>{
      if(a.id!==actId)return a;
      const already=a.requests?.find(r=>r.user===user.display);
      if(already)return a;
      return {...a,requests:[...(a.requests||[]),{user:user.display,role:user.role,date,status:"pending"}]};
    });
    LS.saveActivities(updated); sa(updated); stt("ស្នើសុំបានផ្ញើ!");
  };

  const approveReq=(actId,reqUser,approve)=>{
    const updated=acts.map(a=>{
      if(a.id!==actId)return a;
      return{...a,requests:a.requests.map(r=>r.user===reqUser?{...r,status:approve?"approved":"rejected"}:r)};
    });
    LS.saveActivities(updated); sa(updated); stt(approve?"អនុម័តរួចហើយ!":"បានបដិសេធ!");
  };

  if(acts.length===0) return <div className="ibox">មិនទាន់មានផែនការផ្សព្វផ្សាយ</div>;

  return(<>{toast&&<Toast msg={toast} onDone={()=>stt("")}/>}
    {acts.map(a=>(
      <div key={a.id} className="act-card">
        <div style={{display:"flex",alignItems:"center",gap:8,flexWrap:"wrap"}}>
          <span className={`act-type ${a.type}`}>{a.type==="booth"?"🏪 Booth":"🚗 Roadshow"}</span>
          <span style={{fontSize:10,color:"var(--t3)"}}>{a.date}</span>
        </div>
        <div className="act-title">{a.title}</div>
        <div className="act-meta">
          <span>📍 {a.location} · {a.village} · {a.commune} · {a.district}</span>
          <span>👤 {a.staffName} ({roleConfig[a.staffRole]?.label||a.staffRole})</span>
        </div>
        {/* Members approved */}
        {(a.requests||[]).filter(r=>r.status==="approved").length>0&&(
          <div>
            <div style={{fontSize:10,color:"var(--t2)",marginBottom:4,fontWeight:700}}>ក្រុមចូលរួម</div>
        ...
        {/* Pending requests — only for approvers */}
        {(canEdit||canApprove)&&(a.requests||[]).filter(r=>r.status==="pending").length>0&&(
          <SBox label="សំណើរចូលរួម">
            {(a.requests||[]).filter(r=>r.status==="pending").map(r=>(
              <div key={r.user} className="req-row">
                ...
                <button className="btn-approve" onClick={()=>approveReq(a.id,r.user,true)}>✓</button>
                <button className="btn-reject" onClick={()=>approveReq(a.id,r.user,false)}>✕</button>
              </div>
            ))}
          </SBox>
        )}
        {/* Join button for loan officers */}
        {canJoin&&(()=>{
          const myReq=(a.requests||[]).find(r=>r.user===user.display);
          if(myReq) return <span className={`status-badge ${myReq.status==="pending"?"status-pending":myReq.status==="approved"?"status-approved":"status-rejected"}`}>{myReq.status==="pending"?"⏳ រង់ចាំអនុម័ត":myReq.status==="approved"?"✅ បានអនុម័ត":"❌ បានបដិសេធ"}</span>;
          return <button className="btn btn-purple btn-sm" onClick={()=>requestJoin(a.id)}>📋 Request Join Activity</button>;
        })()}
      </div>
    ))}
  </>);
...
function DBMC({user}){
  ...
  useEffect(()=>{sr(LS.getReports());sa(LS.getActivities())},[tab]);
  ...
  const submitReport=async()=>{
    ...
    LS.saveReport({type:"DBMC",user:user.display,data:f,msg});
  ...
  const createActivity=()=>{
    if(!aForm.title||!aForm.date||!aForm.location)return stt("សូមបំពេញគ្រប់ field!");
    const newAct={...aForm,id:Date.now(),requests:[],createdBy:user.display,createdAt:new Date().toISOString()};
    const updated=[...acts,newAct];
    LS.saveActivities(updated); sa(updated);
  ...
  return(<>{toast&&<Toast msg={toast} onDone={()=>stt("")}/>}
    <div className="sb">
      <div className="tabs">
        {[["dash","📊 Dashboard"],["report","📝 Report"],["plan","📣 ផ្សព្វផ្សាយ"]].map(([k,l])=>(
          <button key={k} className={`tab ${tab===k?"on":""}`} onClick={()=>st(k)}>{l}</button>))}
      </div>
      ...
      {tab==="dash"&&(<>
        <div className="sg">
          {[
            {lbl:"Teller",val:byType("Teller"),color:"#60a5fa"},
            {lbl:"CSA",val:byType("CSA")+byType("CSA_Officer"),color:"#34d399"},
            {lbl:"Loan",val:byType("Loan")+byType("Loan_Officer"),color:"#a78bfa"},
            {lbl:"Total",val:rpts.length,color:"#fbbf24"},
          ].map(x=>(
            <div key={x.lbl} className="sc">
              <div className="sn" style={{color:x.color}}>{x.val}</div>
              <div className="sl">{x.lbl} Reports</div>
            </div>
          ))}
        </div>
        <ReportList reports={rpts} showFull={true}/>
      ...
      {tab==="report"&&(
        <GC icon="📊" title="DBMC Report" sub="Branch Summary" accent="#fb923c">
          ...
          <div className="r2">
            <F label="Branch CIF" ph="ចំនួន..." val={f.cif} set={v=>s("cif",v)} nb/>
            <F label="Branch KHQR" ph="ចំនួន..." val={f.khqr} set={v=>s("khqr",v)} nb/>
          </div>
          <SBox label="B_Vault">
            <div className="r2">
              <F label="Vault USD" ph="0" val={f.vU} set={v=>s("vU",v)} nb/>
              <F label="Over Limit USD" ph="0" val={f.vUL} set={v=>s("vUL",v)} nb/>
            </div>
            <div className="r2">
              <F label="Vault KHR" ph="0" val={f.vK} set={v=>s("vK",v)} nb/>
              <F label="Over Limit KHR" ph="0" val={f.vKL} set={v=>s("vKL",v)} nb/>
            </div>
          </SBox>
      ...
      {tab==="plan"&&(<>
        <GC icon="➕" title="បង្កើតផែនការផ្សព្វផ្សាយ" sub="Booth / Roadshow" accent="#fb923c">
          <F label="ឈ្មោះសកម្មភាព" ph="ឧ. ផ្សព្វផ្សាយ KHQR..." val={aForm.title} set={v=>af("title",v)} icon="📣"/>
          <div className="r2">
            <Sel label="ប្រភេទ" val={aForm.type} set={v=>af("type",v)} opts={[["booth","🏪 Booth"],["roadshow","🚗 Roadshow"]]}/>
            <F label="ថ្ងៃ" ph="" val={aForm.date} set={v=>af("date",v)} nb type="date"/>
          </div>
          <F label="ទីតាំង / Location" ph="ឧ. ផ្សារថ្មី..." val={aForm.location} set={v=>af("location",v)} icon="📍"/>
          <div className="r2">
            <F label="ភូមិ" ph="ឈ្មោះភូមិ..." val={aForm.village} set={v=>af("village",v)} nb/>
            <F label="ឃុំ" ph="ឈ្មោះឃុំ..." val={aForm.commune} set={v=>af("commune",v)} nb/>
          </div>
          <F label="ស្រុក / District" ph="ឈ្មោះស្រុក..." val={aForm.district} set={v=>af("district",v)} nb/>
          <div className="div"/>
        ...
        <div style={{fontSize:12,fontWeight:700,color:"var(--t1)"}}>📋 ផែនការទាំងអស់</div>
        <ActivityViewer user={user} canEdit={true} canApprove={true}/>
      </>)}
    </div></>);
}
...
function BM({user}){
  const [tab,st]=useState("dash");
  const [rpts,sr]=useState(LS.getReports);
  const [toast,stt]=useState("");

  useEffect(()=>sr(LS.getReports()),[tab]);
  ...
  return(<>{toast&&<Toast msg={toast} onDone={()=>stt("")}/>}
    <div className="sb">
      <div className="tabs">
        {[["dash","📊 Reports"],["plan","📣 ផ្សព្វផ្សាយ"]].map(([k,l])=>(
          <button key={k} className={`tab ${tab===k?"on":""}`} onClick={()=>st(k)}>{l}</button>))}
      </div>
      {tab==="dash"&&(<>
        <div className="sg">
      ...
      {tab==="plan"&&(<>
        <div style={{fontSize:12,fontWeight:700,color:"var(--t1)"}}>📋 ផែនការផ្សព្វផ្សាយ</div>
        <ActivityViewer user={user} canEdit={false} canApprove={true}/>
      </>)}
    </div></>);
}
  ...
  return(
    <GC icon="🔑" title="ប្ដូរ Password User" sub="Reset Password · SHA-256" accent="#fbbf24">
      <div style={{background:"rgba(251,146,60,.08)",border:"1px solid rgba(251,146,60,.2)",borderRadius:10,padding:"10px 13px",fontSize:11,color:"#fb923c",lineHeight:1.7}}>
        🔐 Password ថ្មីនឹងត្រូវបាន <b>Hash SHA-256</b> ភ្លាមៗ — គ្មាននរណាម្នាក់ឃើញ Plaintext ទេ រួមទាំង Developer
      </div>
      <div className="field">
        <label>ជ្រើស User</label>
      ...
      <F label="Password ថ្មី" ph="យ៉ាងតិច 8 តួអក្សរ" val={np} set={snp} icon="🔒" type="password"/>
      <F label="បញ្ជាក់ Password" ph="Type again..." val={np2} set={snp2} icon="🔒" type="password"/>
      {err&&<div className="ebox">{err}</div>}
...
function Admin({onBgChange}){
  ...
  const [users,su]=useState(LS.getUsers);
  const [rpts]=useState(LS.getReports);
  const [nu,snu]=useState({username:"",password:"",role:"supervisor_teller",display:""});
  const [toast,stt]=useState("");
  const stored=LS.getBg();
  const [selId,setSel]=useState(stored.id||"cosmos");
  ...
  const applyCustom=()=>{
    const bg={id:"custom",label:"Custom",a:c1,b:c2,c:"#000",accent:c1};
    setSel("custom"); LS.saveBg(bg); onBgChange(bg); stt("Background ផ្ទាល់ខ្លួនបានអនុវត្ត!");
  };

  return(<>{toast&&<Toast msg={toast} onDone={()=>stt("")}/>}
    <div className="sb">
      <div className="tabs">
        {[["users","👥 Users"],["reports","📋 Reports"],["new","➕ Add"],["theme","🎨 Theme"],["pw","🔑 Password"]].map(([k,l])=>(
          <button key={k} className={`tab ${tab===k?"on":""}`} onClick={()=>st(k)}>{l}</button>))}
      </div>
      ...
      {tab==="users"&&(
        <div className="gc">
          <div className="gch">
            <div className="gci" style={{background:"color-mix(in srgb,#fbbf24 14%,transparent)",borderColor:"color-mix(in srgb,#fbbf24 25%,transparent)"}}>👥</div>
            <div><div className="gct">Users ទាំងអស់</div><div className="gcs">{users.length} accounts</div></div>
          </div>
          {users.map(u=>(
            <div key={u.id} className="ui">
              ...
              <div className="uinf">
                <div className="un">{u.display}</div>
                <div className="uro">@{u.username} · {roleConfig[u.role]?.label||u.role}</div>
              </div>
      ...
      {tab==="reports"&&<ReportList reports={rpts} showFull={true}/>}

      {tab==="new"&&(
        <GC icon="➕" title="បង្កើត User ថ្មី" accent="#34d399">
          <F label="ឈ្មោះបង្ហាញ" ph="ឧ. Loan Officer B..." val={nu.display} set={v=>snu(p=>({...p,display:v}))} icon="✏️"/>
          <div className="r2">
            <F label="Username" ph="username" val={nu.username} set={v=>snu(p=>({...p,username:v}))} nb/>
            <F label="Password" ph="••••••" val={nu.password} set={v=>snu(p=>({...p,password:v}))} nb type="password"/>
          </div>
      ...
      {tab==="theme"&&(
        <GC icon="🎨" title="ប្ដូរ Background" sub="Admin Only · Live Preview" accent="#fbbf24">
          ...
          <div className="slbl" style={{marginTop:4}}>Custom Gradient</div>
          <div className="cprow">
            <div className="cpw"><label>ពណ៌ទី ១</label><input type="color" className="cinp" value={c1} onChange={e=>sc1(e.target.value)}/></div>
            <div className="cpw"><label>ពណ៌ទី ២</label><input type="color" className="cinp" value={c2} onChange={e=>sc2(e.target.value)}/></div>
          </div>
          <div className="cprev" style={{background:`linear-gradient(135deg,${c1},${c2})`}}/>
          <button className="btn btn-green" onClick={applyCustom}>✅ អនុវត្ត Background</button>
          <div className="note">ការប្ដូរ Background នឹងមានឥទ្ធិពលភ្លាមៗ</div>
        </GC>
      )}

      {tab==="pw"&&<ResetPassword users={users} onUpdate={(upd)=>{su(upd);LS.saveUsers(upd);stt("Password បានធ្វើបច្ចុប្បន្នភាព ✅");}}/>}
    </div></>);
}
...
export default function App(){
  ...
  const [needSetup,sns]=useState(()=>{
    const users=LS.getUsers();
    return users.length===0||!users.some(u=>u.role==="admin");
  });
